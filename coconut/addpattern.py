...
def factorial(_coconut_match_first_arg=_coconut_sentinel, *_coconut_match_args, **_coconut_match_kwargs):  #1 (line in Coconut source)
    ...
    if not _coconut_match_check_0:  #1 (line in Coconut source)
        raise _coconut_FunctionMatchError('def factorial(0) = 1', _coconut_match_args)  #1 (line in Coconut source)

    return 1  #1 (line in Coconut source)


try:  #3 (line in Coconut source)
    _coconut_addpattern_0 = _coconut_addpattern(factorial)  # type: ignore  #3 (line in Coconut source)
except _coconut.NameError:  #3 (line in Coconut source)
    _coconut.warnings.warn("Deprecated use of 'addpattern def factorial' with no pre-existing 'factorial' function (use 'match def factorial' for the first definition or switch to 'case def' syntax)", _coconut_CoconutWarning)  #3 (line in Coconut source)
    _coconut_addpattern_0 = lambda f: f  #3 (line in Coconut source)
@_coconut_addpattern_0  #3 (line in Coconut source)
...
def factorial(_coconut_match_first_arg=_coconut_sentinel, *_coconut_match_args, **_coconut_match_kwargs):  #3 (line in Coconut source)
    """Compute n! where n is an integer >= 0."""  #4 (line in Coconut source)
    _coconut_match_check_1 = False  #5 (line in Coconut source)
    _coconut_match_set_name_n = _coconut_sentinel  #5 (line in Coconut source)
    ...
    if (_coconut.len(_coconut_match_args) <= 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "n" in _coconut_match_kwargs)) == 1):  #5 (line in Coconut source)
        _coconut_match_temp_0 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("n")  #5 (line in Coconut source)
        _coconut_match_set_name_n = _coconut_match_temp_0  #5 (line in Coconut source)
        _coconut_match_temp_1 = _coconut.getattr(int, "_coconut_is_data", False) or _coconut.isinstance(int, _coconut.tuple) and _coconut.all(_coconut.getattr(_coconut_x, "_coconut_is_data", False) for _coconut_x in int)  # type: ignore  #5 (line in Coconut source)
        if not _coconut_match_kwargs:  #5 (line in Coconut source)
            _coconut_match_check_1 = True  #5 (line in Coconut source)
    if _coconut_match_check_1:  #5 (line in Coconut source)
        _coconut_match_check_1 = False  #5 (line in Coconut source)
        if not _coconut_match_check_1:  #5 (line in Coconut source)
            if (_coconut_match_temp_1) and (_coconut.isinstance(_coconut_match_temp_0, int)):  #5 (line in Coconut source)
                _coconut_match_temp_2 = _coconut.len(_coconut_match_temp_0) <= _coconut.max(0, _coconut.len(_coconut_match_temp_0.__match_args__)) and _coconut.all(i in _coconut.getattr(_coconut_match_temp_0, "_coconut_data_defaults", {}) and _coconut_match_temp_0[i] == _coconut.getattr(_coconut_match_temp_0, "_coconut_data_defaults", {})[i] for i in _coconut.range(0, _coconut.len(_coconut_match_temp_0.__match_args__))) if _coconut.hasattr(_coconut_match_temp_0, "__match_args__") else _coconut.len(_coconut_match_temp_0) == 0  # type: ignore  #5 (line in Coconut source)
                if _coconut_match_temp_2:  #5 (line in Coconut source)
                    _coconut_match_check_1 = True  #5 (line in Coconut source)

        if not _coconut_match_check_1:  #5 (line in Coconut source)
            if (not _coconut_match_temp_1) and (_coconut.isinstance(_coconut_match_temp_0, int)):  #5 (line in Coconut source)
                _coconut_match_check_1 = True  #5 (line in Coconut source)
            if _coconut_match_check_1:  #5 (line in Coconut source)
                _coconut_match_check_1 = False  #5 (line in Coconut source)
                if not _coconut_match_check_1:  #5 (line in Coconut source)
                    if _coconut.type(_coconut_match_temp_0) in _coconut_self_match_types:  #5 (line in Coconut source)
                        _coconut_match_check_1 = True  #5 (line in Coconut source)

                if not _coconut_match_check_1:  #5 (line in Coconut source)
                    if not _coconut.type(_coconut_match_temp_0) in _coconut_self_match_types:  #5 (line in Coconut source)
                        _coconut_match_temp_3 = _coconut.getattr(int, '__match_args__', ())  # type: _coconut.typing.Any  # type: ignore  #5 (line in Coconut source)
                        if not _coconut.isinstance(_coconut_match_temp_3, _coconut.tuple):  #5 (line in Coconut source)
                            raise _coconut.TypeError("int.__match_args__ must be a tuple")  #5 (line in Coconut source)
                        if _coconut.len(_coconut_match_temp_3) < 0:  #5 (line in Coconut source)
                            raise _coconut.TypeError("too many positional args in class match (pattern requires 0; 'int' only supports %s)" % (_coconut.len(_coconut_match_temp_3),))  #5 (line in Coconut source)
                        _coconut_match_check_1 = True  #5 (line in Coconut source)
    ...
    if _coconut_match_check_1:  #5 (line in Coconut source)
        if _coconut_match_set_name_n is not _coconut_sentinel:  #5 (line in Coconut source)
            n = _coconut_match_set_name_n  #5 (line in Coconut source)
    if _coconut_match_check_1 and not (n > 0):  #5 (line in Coconut source)
        _coconut_match_check_1 = False  #5 (line in Coconut source)
    if not _coconut_match_check_1:  #5 (line in Coconut source)
        raise _coconut_FunctionMatchError('addpattern def factorial(int() as n if n > 0) =', _coconut_match_args)  #5 (line in Coconut source)

    return _coconut_tail_call((reduce), _coconut.operator.mul, (range(1))(n + 1))  #5 (line in Coconut source)

# Test cases:

cases = [-1, 0.5, 0, 3]  #8 (line in Coconut source)
for case in cases:  #9 (line in Coconut source)
    try:  #10 (line in Coconut source)
        (print)((factorial)(case))  #11 (line in Coconut source)
    except MatchError as e:  #12 (line in Coconut source)
        (print)(e)  #13 (line in Coconut source)
