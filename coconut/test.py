...
def factorial(n):  #1 (line in Coconut source)
    """Compute n! where n is an integer >= 0."""  #2 (line in Coconut source)
    if (isinstance)(n, int) and n >= 0:  #3 (line in Coconut source)
        acc = 1  #4 (line in Coconut source)
        for x in range(1, n + 1):  #5 (line in Coconut source)
            acc *= x  #6 (line in Coconut source)
        return acc  #7 (line in Coconut source)
    else:  #8 (line in Coconut source)
        raise TypeError("the argument to factorial must be an integer >= 0")  #9 (line in Coconut source)

# Test cases:

cases = [-1, 0.5, 0, 3]  #12 (line in Coconut source)
for case in cases:  #13 (line in Coconut source)
    try:  #14 (line in Coconut source)
        (print)((factorial)(case))  #15 (line in Coconut source)
    except TypeError as e:  #16 (line in Coconut source)
        (print)(e)  #17 (line in Coconut source)
